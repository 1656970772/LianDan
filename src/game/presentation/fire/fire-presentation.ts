...
import type { FirePresentationSource } from './contracts.ts'
import type { FirePresentationConfig } from './fire-presentation-config.ts'
...
export type FireParticleView = Readonly<{
  count: number
  x: Float32Array
  y: Float32Array
  flowX: Float32Array
...
export class FirePresentation {
  readonly #config: FirePresentationConfig
  ...
  readonly #particleView: FireParticleView
  #frame = 0
  #revealDistancePixels: number | null = null

  constructor(config: FirePresentationConfig) {
    this.#config = config
  ...
  reset(view: FireFlowReadView, source: FirePresentationSource): void {
    this.#resetParticles(view, source)
    this.#revealDistancePixels =
      this.#config.startup.mode === 'rapid-reveal' ? 0 : null
  }

  resetSteady(view: FireFlowReadView, source: FirePresentationSource): void {
    this.#resetParticles(view, source)
    this.#revealDistancePixels = null
  }
  ...
  advance(
    ...
    if (
      this.#revealDistancePixels !== null &&
      this.#config.startup.mode === 'rapid-reveal'
    ) {
      this.#revealDistancePixels +=
        this.#config.startup.propagationSpeedPixelsPerSecond * elapsedSeconds
    }
    const delta = Math.max(
      0,
      Math.min(this.#config.maximumDeltaSeconds, elapsedSeconds),
    )
    for (let index = 0; index < this.#x.length; index += 1) {
  ...
  get particles(): FireParticleView {
  ...
  get revealDistancePixels(): number | null {
    return this.#revealDistancePixels
  }

  #spawn(index: number, source: FirePresentationSource): void {
