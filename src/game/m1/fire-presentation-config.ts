export type M1FirePresentationConfig = Readonly<{
  ...
  heatField: M1FireHeatFieldConfig
  sparks: Readonly<{
    stride: number
    radiusPixels: number
    alpha: number
...
export type M1FireHeatPaletteStop = Readonly<{
  heat: number
  red: number
  green: number
  blue: number
...
export type M1FireHeatFieldConfig = Readonly<{
  width: number
  height: number
  pixelScale: number
  ...
  palette: Readonly<{
    outer: M1FireHeatPaletteStop
    middle: M1FireHeatPaletteStop
    core: M1FireHeatPaletteStop
  }>
}>

export const M1_FIRE_PRESENTATION_CONFIG: M1FirePresentationConfig =
  Object.freeze({
    particleCount: 280,
