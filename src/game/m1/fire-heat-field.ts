...
import type { M1FireHeatFieldConfig } from './fire-presentation-config.ts'
import type { M1FireParticleView } from './fire-presentation.ts'

export type M1FireHeatFrame = Readonly<{
  width: number
  height: number
  pixels: Uint8ClampedArray<ArrayBuffer>
}>

export interface M1FireOcclusionInput {
 ...
 */
export class M1FireHeatField {
  readonly #config: M1FireHeatFieldConfig
  ...
  readonly #frame: M1FireHeatFrame

  constructor(config: M1FireHeatFieldConfig) {
    this.#config = config
  ...
  render(
    ...
    particles: M1FireParticleView,
    source: M1FixtureSource,
    occlusion?: M1FireOcclusionInput,
  ): M1FireHeatFrame {
    this.#density.fill(0)
    ...
    this.#writePixels(view, source, occlusion !== undefined)
    return this.#frame
  }

  #splatSource(source: M1FixtureSource): void {
  ...
  #rasterizeOcclusion(input: M1FireOcclusionInput): void {
    const scale = this.#config.pixelScale
    this.#occlusion.fill(0)
